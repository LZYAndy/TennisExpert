...
from .custom.qwen3_vl import Qwen3VLForConditionalGeneration as CustomQwen3VLForConditionalGeneration
        ...
        if model_args.mixture_of_depths == "load":
            ...
            if getattr(config, "model_type", None) == "qwen3_vl":
                if model_args.train_from_scratch:
                    model = CustomQwen3VLForConditionalGeneration(config)
                else:
                    custom_init_kwargs = dict(init_kwargs)
                    pretrained_model_name_or_path = custom_init_kwargs.pop("pretrained_model_name_or_path")
